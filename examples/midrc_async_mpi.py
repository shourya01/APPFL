...
def get_data(comm: MPI.Comm):
    dir = os.getcwd() + "/datasets/MIDRC_Data"

    comm_rank = comm.Get_rank()

    train_data = []
    test_data = []
    with open(dir+"/../MIDRC_Imaging_Studies_table.csv", 'r') as file:
        csvreader = csv.reader(file)
        header = next(csvreader)
        for row in csvreader:
            # only IL state zip code starts with 6 (in IL, NC, CA, IN, TX)
            img_path = dir +'/'+ row[12]
            class_name = int(row[21]=='Yes')
            file_names = os.listdir(img_path)
            # print(row[12])
            for file_name in file_names:
                if file_name.endswith('.png'):
                    if row[10][0]=="6":  
                        test_data.append([img_path+'/'+file_name, class_name])
                    if row[10][0]=='2':
                        train_data.append([img_path+'/'+file_name, class_name, "NC"])
                    if row[10][0]=='9':
                        train_data.append([img_path+'/'+file_name, class_name, "CA"])
                    if row[10][0]=='4':
                        train_data.append([img_path+'/'+file_name, class_name, "IN"])
                    if row[10][0]=='7':
                        train_data.append([img_path+'/'+file_name, class_name, "TX"])
    ...
    for idx in range(len(test_data)):
        ...
        test_data_label.append(class_name)
...
# mpiexec -np n+1 --mca opal_cuda_support 1 python ./midrc_async_mpi.py
# To run MPI with n clients:
# mpiexec -np n+1 python ./midrc_async_mpi.py
